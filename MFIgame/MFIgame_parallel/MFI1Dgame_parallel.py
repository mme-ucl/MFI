...
def absolute_error(y, FES, nbins):
    AD = abs(FES - y)
    AAD = sum(AD) / nbins
    return [AD, AAD]
...
def patch_FES_AD_ofe(force_vector, grid, y, nbins):
    ...
    print("shape(force_vector):", np.shape(force_vector))
    ...
    F_patch = np.divide(F_patch, PD_patch, out=np.zeros_like(F_patch), where=PD_patch > 10**-5)

    #Calculate error
    PD_ratio = np.divide(PD2_patch, (PD_patch ** 2 - PD2_patch), out=np.zeros_like(PD_patch), where=(PD_patch ** 2 - PD2_patch) > 0)
    OFV = (np.divide(OFV_patch, PD_patch, out=np.zeros_like(OFV_patch), where=PD_patch > 10**-5) - F_patch ** 2) * PD_ratio
    OFE = np.sqrt(OFV)
    AOFE = sum(OFE) / nbins
