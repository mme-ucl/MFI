...
param_extream = [[25000, 300000], [-1.75, 1.75], [0.05, 0.15], [0.5, 3], [1, 16]]
...
class Agent:
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 50) < self.epsilon:
            length_rand = int(25000 * (1 + random.randint(0, 11)))  # 25000 * (1 + [0 -- 11]) => [25_000, 300_000]
            ...
            #predictions_range = [[25_000, 300_000], [-1.75, 1.75], [0.05, 0.15], [0.5, 3], [1, 16]]
            prediction = [25000 * int(1 + pred[0]*11.99), pred[1]*3.5 - 1.75, 0.05 + pred[2]*0.1, 0.5 + pred[3]*2.5, 1 + pred[4]*15]
...
def train():
    ...
    for sim_itteration in range(100):
        ...
        if done == 1:
            ...
            record = AAD
            agent.update_progress(AAD, Aofe, x, y, Ftot_den, FES, ofe, [25000, 0.0, 0.1, 1, 10], record)
        ...
        if done == 1:           
            ...
            print('Game', agent.n_games, 'finished --> AAD', round(AAD,2), ' | Aofe:', round(Aofe,2), ' | reward:', int(reward),  ' | time:', round(time.time() - start,2))
            agent.n_games += 1         
        
        agent.update_progress(AAD, Aofe, x, y, Ftot_den, FES, ofe, final_move, record)
